...
const MCQAllMain = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      const res = await getAllPapers({isApproved: "Yes"});
      setPapers(res?.data || []);
    };
  ...
  const filteredPapers = papers.filter((paper) => {
    const matchesSubject =
      filters.subject.length === 0 || filters.subject.includes(paper.subject);
    const matchesExam =
    ...
    const matchesType =
      filters.type.length === 0 || filters.type.includes(paper.type);
    const matchesSearch =
      searchText === "" ||
      paper.longName.toLowerCase().includes(searchText.toLowerCase()); // Search by name

    return matchesSubject && matchesExam && matchesMedium && matchesFee && matchesType && matchesSearch;
  });
