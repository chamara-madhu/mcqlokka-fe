export const HOME_PATH = "/";
export const PAPERS_BY_SUBJECTS_PATH = "/subjects/:subjectId";


export const MCQ_ALL_PATH = "/mcq";
export const MCQ_PAPER_PATH = "/mcq/:paperId";
export const MCQ_EXAM_PATH = "/mcq/exam/:paperId";
export const MCQ_EXAM_LEARNING_PATH = "/mcq/exam/learning/:paperId";
export const MCQ_EXAM_MARK_PATH = "/mcq/exam/marks/:markId";
export const MCQ_BUY_PAPER_PATH = "/mcq/buy/paper/:paperId";
export const PAYMENT_SUCCESS_PATH = "/payment/success/:paperId";

export const MCQS_BY_LESSON_PATH = "/mcqs/by/lesson";

export const CART_PATH = "/cart";
export const CHECKOUT_PATH = "/checkout";
export const ABOUT_PATH = "/about";
export const CONTACT_PATH = "/contact";
export const PRICING_PATH = "/pricing";
export const FAQ_PATH = "/contact#faq";
export const LOGIN_PATH = "/login";
export const REGISTER_PATH = "/register";
export const VERIFY_ACCOUNT_PATH = "/verify/:token";
export const CONTACT_US_PATH = "/contact";

export const ADMIN_DASHBOARD_PATH = "/admin/dashboard";
export const ADMIN_SUBJECT_CREATE_PATH = "/admin/subjects/create";
export const ADMIN_SUBJECT_EDIT_PATH = "/admin/subjects/create/:id";
export const ADMIN_SUBJECT_MANAGE_PATH = "/admin/subjects/manage";
export const ADMIN_PAPER_CREATE_PATH = "/admin/papers/create";
export const ADMIN_PAPER_EDIT_PATH = "/admin/papers/create/:id";
export const ADMIN_PAPER_MANAGE_PATH = "/admin/papers/manage";
export const ADMIN_LESSON_CREATE_PATH = "/admin/lessons/create";
export const ADMIN_LESSON_EDIT_PATH = "/admin/lessons/create/:id";
export const ADMIN_LESSON_MANAGE_PATH = "/admin/lessons/manage";
export const ADMIN_QUESTION_CREATE_PATH = "/admin/questions/create";
export const ADMIN_EXTRACT_QUESTION_PATH = "/admin/questions/ai-extract";
export const ADMIN_GENERATE_MODEL_QUESTION_PATH = "/admin/questions/ai-model-paper-generation";
export const ADMIN_QUESTION_EDIT_PATH = "/admin/questions/create/:id";
export const ADMIN_QUESTION_MANAGE_PATH = "/admin/questions/manage";
export const ADMIN_ALL_USERS_PATH = "/admin/reports/all-users";
export const ADMIN_ALL_PAYMENTS_PATH = "/admin/reports/all-payments";

export const MY_PROFILE_PATH = "/my-profile";
export const MY_PURCHASING_HISTORY_PATH = "/my-profile/purchasing-history";
export const MY_RESULTS_PATH = "/my-profile/results";
export const MY_SUBJECTS_PATH = "/my-profile/subjects";
